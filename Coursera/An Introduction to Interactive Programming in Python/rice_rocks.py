...
def keydown(key):
    ...
    elif key == simplegui.KEY_MAP['space'] and missiles > 0:
        my_ship.shoot() 
        missiles -= 1
...
# drawn
def click(pos):
    global started, lives, score, soundtrack, missiles
...
def draw(canvas):
    ...
    for rock in rock_group:
        for i in range(2):
            rock.vel[i] = rock.init_vel[i] + (rock.init_vel[i] * score * 3 * 0.03)
    ...
    if group_collide(rock_group, my_ship):
        lives -= 1
    score += group_group_collide(missile_group, rock_group) * 0
    ...
    canvas.draw_text("Missiles", [370, 50], 22, "White", "sans-serif")
    canvas.draw_text("Score", [680, 50], 22, "White", "sans-serif")
    canvas.draw_text(str(lives), [68, 80], 22, "White", "sans-serif")
    canvas.draw_text(str(score), [702, 80], 22, "White", "sans-serif")
    canvas.draw_text(str(missiles), [400, 80], 22, "White", "sans-serif")
...
score_timer = simplegui.create_timer(500, score_update)
